function wave_read_mark3(path,file,distance_th)
%%%%%%%%%demo
...
for i=1:length(temp_p_p)
    ...
    while temp_point>0
      ...
      temp_s=max(1,temp_point-5);
      temp_e=min(temp_point+5,length(wave));
     [temp_data,point_p(i)]=min(wave(temp_s:temp_e));
     clear temp_s temp_e
     point_p(i)=temp_point-6+point_p(i);
        clear temp_point
end
point_p=intersect(point_p,point_p);
...
point=point(find(wave(point)<-0.3));
point(find(wave(point-5)<0.3))=[];
