function [Nex Lag]=filter_SMR(PathFileName,ChanParameter,FilterParamter,NexFile)
...
%    if ~isempty(RewardT)
%       
%       Index=ceil(RewardT/freq);
%       Index=Index(:);
%       
%       for ii=1:length(Index)
%           Invalid=Index(ii):(Index(ii)+freq*1);
%           data(Invalid)=0;
% 
%       end
%        
...
%     if FilterParamter.CereDelta       %filter data in delta band
%        Nex.contvars{end+1,1}.data=FilterDelta(data,freq,FilterParamter.CereDeltaOrder)';
%        Nex.contvars{end,1}.name=['CereCh' num2str(Chan) 'Delta'];
%        Nex.contvars{end,1}.varVersion=100;
%        Nex.contvars{end,1}.MVOfffset=0;
%        Nex.contvars{end,1}.ADFrequency=freq;
%        Nex.contvars{end,1}.timestamps=0;
