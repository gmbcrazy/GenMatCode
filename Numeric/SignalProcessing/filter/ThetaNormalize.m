...
[sf_theta,filtwts] = eegfilt(signal(:)',Freq,lowf,highf,0,filterorder,0);
        ...
        shift_step=4*Freq/10;
        give_up_length=4*Freq;    %abandon the theta wave whose length is smaller than give_up_length
        shift_num=floor((length(sf_theta)-2*Freq)/shift_step);
        mark_theta=zeros(1,shift_num);
        for jj=1:shift_num
            sta_jj=(jj-1)*shift_step+1;
            fin_jj=sta_jj+2*Freq;
...
TEMP_length_standard=(give_up_length-Freq*2)/shift_step+1;
...
norm_tsf(((TEMP_theta_validindex(j)-1)*shift_step+1):((TEMP_theta_validindex(j)-1)*shift_step+Freq*2))=sf_theta(((TEMP_theta_validindex(j)-1)*shift_step+1):((TEMP_theta_validindex(j)-1)*shift_step+Freq*2));
norm_phase_sf(((TEMP_theta_validindex(j)-1)*shift_step+1):((TEMP_theta_validindex(j)-1)*shift_step+Freq*2))=phase_sf(((TEMP_theta_validindex(j)-1)*shift_step+1):((TEMP_theta_validindex(j)-1)*shift_step+Freq*2));
