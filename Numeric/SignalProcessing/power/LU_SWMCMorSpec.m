function [sample,ThetaTS,varargout] = LU_SWMCMorSpec(LFP,LFPtheta,thetaphase,samprate,timerange,varargin)

%%%%extract slow wave modulated(SWM) spectrum for each single slow wave circle;
%%%%spectrum is calcuated using comples morlet wavelet 
...
%%%%%%%%%previously used with smoothing
% cfs1 = smoothCFS(invscales.*abs(cwtS1.cfs).^2,scales,dt,length(scales));
% % cfs1 = smoothCFS(abs(cwtS1.cfs).^2,scales,dt,length(scales));
% cfs1=sqrt(cfs1);
%%%%%%%%%previously used with smoothing


% flag_SMOOTH=true;
% cfs1 = smoothCFS(abs(cwtS1.cfs).^2,flag_SMOOTH,nsw,ntw);
% cwtS1.cfs = sqrt(cfs1);
% 
