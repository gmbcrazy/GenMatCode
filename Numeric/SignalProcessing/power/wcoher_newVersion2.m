function varargout = wcoher_newVersion2(s1,s2,scales,wname,dt,varargin)
%%%%%%%%
%%this is agorithm of coherence from Mathworks function wcoherence.m
%%but the useage is the same as the old Mathworks function wcoher.m
%%Lu Zhang, 2/22/2019.
...
if nbIN>5
    ...
    while k<=nbIN
        ...
        switch upper(argNAM(1:3))
            ...
            case 'ASC' , ASC = argVAL;
            case 'QUI' , QUI = argVAL;
% %             case 'SAM' , dt = argVAL;          %%%%%%%%Sample rate;         
...
% % wname = 'morl';
scales=scales(:);
cwtx = cwtft({s1(:)',dt},'wavelet',wname,'scales',scales);
cwty = cwtft({s2(:)',dt},'wavelet',wname,'scales',scales);
...
cfs_s1 = smoothCFS(invscales.*abs(cfs_s10).^2,scales,dt,NSW);
cfs_s2 = smoothCFS(invscales.*abs(cfs_s20).^2,scales,dt,NSW);
cfs_cross = cfs_s10.*conj(cfs_s20);
cfs_cross = smoothCFS(invscales.*cfs_cross,scales,dt,NSW);
WCOH = (cfs_cross).^2./(cfs_s1.*cfs_s2);
N = size(cfs_s1,2);




varargout = {WCOH,cfs_cross,cfs_s1,cfs_s2};
...
if OK_all || OK_cwt
    STR1 = sprintf('CWT for %s and %s',inputname(1),inputname(2));
...
function cfs = smoothCFS(cfs,scales1,dt,ns)
N = size(cfs,2);
npad = 2.^nextpow2(N);
omega = 1:fix(npad/2);
