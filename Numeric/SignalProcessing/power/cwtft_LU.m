function CWTStruct = cwtft_LU(SIG,varargin)
...
%%%%%%%%%%%define default smoothing scale No.
if isempty(ns) , ns = 3; end
...
Power = smoothCFS(invscales.*abs(cwtcfs).^2,scales,dt,ns);
...
CWTStruct = struct('cfs',cwtcfs,'Power',Power,'scales',scales, 'frequencies',frequencies,...
