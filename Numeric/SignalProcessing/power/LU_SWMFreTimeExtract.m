function [sample,ThetaTS,varargout] = LU_SWMFreTimeExtract(LFP,LFPtheta,thetaphase,samprate,timerange,SampleDurationLim,varargin)
...
%Output:
%sample is the vectorized spectrum in frequency and time domain;Noted each sample length (in time) varies.
...
% SampleDurationLim: [min max] for sample duration in seconds; any sample with duration outside the interval would be excluded; 
% % Default setting
...
% %     save([SavePath SaveShow '.mat'],'sample','ThetaTS','LFPOutput','WaveParam','PhaseBin','timerange');
    save([SavePath SaveShow '.mat'],'sample','sampleL','ThetaTS','LFPOutput','WaveParam','Fplot','Div','timerange');
...
%%%%%%%%ii loops      multiple theta period in one recording file
    ValidIndex=find(SampleL>=SampleDurationLim(1)*samprateD&SampleL<=SampleDurationLim(2)*samprateD);
    ThetaSpec=ThetaSpec(ValidIndex);
%     ThetaPhase=ThetaPhase(ValidIndex);
    LFPOutput=LFPOutput(ValidIndex);
    SampleL=SampleL(ValidIndex);
    ThetaTimeStamps=ThetaTimeStamps(ValidIndex);
    maxL=max(SampleL);
    ThetaSpecNew=zeros(maxL,size(S1Power,1),length(SampleL))+nan;
   for i=1:length(SampleL)
       ThetaSpecNew(1:SampleL(i),:,i)=ThetaSpec{i};
   end
   clear ThetaSpec;
...
for i=1:size(sample,2)
    ...
    sample(:,i)=sample(:,i)/nansum(sample(:,i))+0.00001;
    sample(:,i)=sample(:,i)/nansum(sample(:,i));
end
