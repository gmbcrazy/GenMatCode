function [sample,ThetaTS,varargout] = LU_SWMcspecExtract(LFP1,LFP2,LFPtheta,thetaphase,samprate,timerange,varargin)

%%%%extract slow wave modulated(SWM) cross-spectrum for each single slow wave circle;


%   Extract Wavelet Cross pectrum for Each Single Theta Circle
...
%LFP: raw LFPs
%LFPtheta: filtered LFPs in slow band
...
% invscales = 1./scales(:);
% invscales = repmat(invscales,1,length(LFPtemp1));

cwtS1 = cwtft({LFPtemp1,dt},'scales',scales,'wavelet','morl');
cwtS2 = cwtft({LFPtemp2,dt},'scales',scales,'wavelet','morl');
scales=cwtS1.scales;
invscales = 1./scales(:);
invscales = repmat(invscales,1,length(LFPtemp1));
...
% 
% cfs2 = smoothCFS(invscales.*abs(cwtS2.cfs).^2,scales,dt,length(scales));
crossCFS = cwtS1.cfs.*conj(cwtS2.cfs);
% crossCFS = smoothCFS(invscales.*crossCFS,scales,dt,length(scales));
crosspec = crossCFS./(sqrt(cwtS1.cfs).*sqrt(cwtS2.cfs));
wtc = abs(crosspec).^2/abs(cwtS1.cfs.*cwtS2.cfs);

[WCOH,WCS,PERIOD,COI] = wcoherence(LFPtemp1,LFPtemp2,samprateD);

imagesc([1:length(LFPtemp1)]/samprateD,log2(PERIOD),WCOH)

bin_width=dt;
Fplot=cwtS1.frequencies;
Splot=cwtS1.scales;
% % % crosspec=abs(cwtS1.cfs);
% % % % WaveParam.Zscore=1;
% % % if WaveParam.Zscore
% % %    crosspec=zscore(crosspec,0,2); %%%%%%%%%%%%%normalize over time
% % % % crosspec=zscore(crosspec,0,1);  %%%%%%%%%%%%%normalize over frequency
% % % end
...
%     ThetaSpec{end+1,1}=crosspec(:,s:o)';
    ThetaSpec{end+1,1}=wtc(:,s:o)';
