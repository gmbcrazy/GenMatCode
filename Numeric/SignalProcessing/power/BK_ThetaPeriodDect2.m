 function [Period,thetaDeltaR,t]=BK_ThetaPeriodDect2(signal,Freq,timeWin,slideWin,tdTh)

%%%%Detect theta period based on theta(5-10Hz)/delta(2-4Hz) > tdTh=4; 
...
%         SlidingTime=shift_step/Freq;
        shift_step=round(slideWin*Freq);
...
% 
%             %     if (sum(Power_theta(temp_low_1:temp_high_1))/sum(Power_theta(temp_low_2:temp_high_2))>tdTh)
%             %         mark_theta(jj)=1;
%             %     end
...
%         Period(2,:)=Period(2,:)*shift_step/Freq+timeWin;
        Period(1,:)=t(Period(1,:))-timeWin/2;
        Period(2,:)=t(Period(2,:))+timeWin/2;

        if length(Period(1,:))>1
          Period=MergePeriod(Period);
        end
...
function Period=CheckPeriod(Period)
