function varargout = wcoher_newVersion(s1,s2,scales,wname,varargin)
%%%%%%%%
%%this is agorithm of coherence from Mathworks function wcoherence.m
%%but in useage as the same of the old Mathworks function wcoher.m
%%Lu Zhang, 2/22/2019.
...
%%%new wcoher 
% % wname = 'morl';
cwtx = cwtft(s1,'wavelet',wname,'scales',scales,'PadMode','symw');
cwty = cwtft(s2,'wavelet',wname,'scales',scales,'PadMode','symw');

cfs_s10 = cwtx.cfs;
cfs_s20 = cwty.cfs;

cfs_s1 = sqrt(smoothCFS(abs(cfs_s10).^2,flag_SMOOTH,NSW,NTW));
cfs_s2 = sqrt(smoothCFS(abs(cfs_s20).^2,flag_SMOOTH,NSW,NTW));
