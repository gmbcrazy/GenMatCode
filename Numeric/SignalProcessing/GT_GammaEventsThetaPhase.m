function [Output,OutputTotal] = GT_GammaEventsThetaPhase(processeddatadir,FileIndex,samprate,varargin)
...
FPlot=F(end:-1:1);
imagesc(Time,FPlot,S1Power);axis xy;hold on;set(gca,'clim',[-3 3]);colormap(jet)
[C,h]=contour(Time,FPlot,S1Power,[4 4],'g');hold on
...
imagesc(C)
colormap(jet)
plot(Time,LFPtemp*20+100,'k');

thRateRatio=
Field=PlaceFieldFind2D(S1Power,thRateRatio,thNum);

filt=fspecial('gaussian', 7,0.5);
edg=2;
res=2;

% % [cent, varargout]=FastPeakFind(d, thres, filt ,edg, res, fid)

cent=FastPeakFind(S1Power,3,filt ,edg, res);
cent=round(cent);

Y=FPlot(cent(2:2:end));
X=Time(cent(1:2:end));
[C,h]=contour(Time,FPlot,S1Power,[3 3],'g');hold on
plot(X,Y,'ro')

thNum=15;
Cell=FieldFind2D_Lu(S1Power,4,thNum);
...
imagesc(Range,FPlot,log2(abs(AlignS1')));axis xy;
if iPlot==PageNum
   set(gca,'ylim',[0;WaveParam.Freq(end)],'clim',[-4 2],'ytick',[0:50:WaveParam.Freq(end)],'box','off','tickdir','out');hold on
else 
   set(gca,'ylim',[0;WaveParam.Freq(end)],'clim',[-4 2],'ytick',[0:50:WaveParam.Freq(end)],'box','off','tickdir','out','xticklabel',[]);hold on
end
...
polar_errorArea(Rbin_Fall(:,iband)'+pi,R_Fall(:,iband)',Rstd_Fall(:,iband)',ColorFbandC,0.3,LimM+2,Rmean_Fall(1,iband)'+pi);hold on
end
...
polar_errorArea(OutputTotal.Rbin(:,iband)+pi,OutputTotal.R(:,iband),OutputTotal.Rstd(:,iband),ColorFbandC,0.3,LimM+2,OutputTotal.Rmean(1,iband)+pi)
end
