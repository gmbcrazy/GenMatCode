function Output = GT_MultiTsTriSpectrum(LFP,NeuronTs,NeuronTsID,samprate,timerange,NeuronID,varargin)
...
%%%%%%%%iCell loops   Multi Cell

for iCell=1:length(NeuronID)

    
    fprintf(['Calculate Spectrum Triggered by Unit ' num2str(iCell) '\n'])
    clear AlignS1 Temp5 TempN Temp2 Temp4
    refTsTemp=NeuronTs(NeuronTsID==NeuronID(iCell));
...
Output(iCell).AlignS1=AlignS1;
Output(iCell).AlighNum=sum(tempN);
Output(iCell).F=FPlot;
Output(iCell).AlignLFP=Temp5;
% % AlignSample(i)=Output(i).AlighNum;
Output(iCell).Duration=sum(Duration);
Output(iCell).NumAnaPeriod=NumAnaPeriod;
