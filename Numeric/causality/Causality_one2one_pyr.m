function Causality=Causality_one2one_pyr(path_filename,Data_name1,Data_name2,timerange,MORDER_band,NFFT,fre_band,fs)
...
for i=1:length(Data_name1)
    if i==1
       [Data1(1,:),time]=rate_historam(path_filename,Data_name1(i).Name,timerange,1/fs,1/fs);
   else
       [temp_data1(1,:),time]=rate_historam(path_filename,Data_name1(i).Name,timerange,1/fs,1/fs);
        Data1=Data1+temp_data1;
   end
end

for i=1:length(Data_name2)
    if i==1
       [Data2(1,:),time]=rate_historam(path_filename,Data_name2(i).Name,timerange,1/fs,1/fs);
   else
       [temp_data2(1,:),time]=rate_historam(path_filename,Data_name2(i).Name,timerange,1/fs,1/fs);
        Data2=Data2+temp_data2;
   end
end
...
for i=MORDER_band(1):MORDER_band(2)
    ...
    Vnames=strvcat(Data_name1(1).Name,Data_name2(1).Name);
    clear temp_n
