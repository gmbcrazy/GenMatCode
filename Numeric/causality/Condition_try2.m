     ...
     Original_data=Original_data+random('Normal',0.1,0.3,1,10000);
     Original_data(2,:)=zeros(1,10000);
     Original_data(3,:)=zeros(1,10000);
    
     rand1=random('Normal',0,0.5,1,length(Original_data));
     rand2=random('Normal',0,0.1,1,length(Original_data));
     rand3=random('Normal',0,0.5,1,length(Original_data)/4);
    ...
    for i=1:length(rand3)
        Original_data(2,i*4)=Original_data(2,i*4)+rand3(i);
    end        

    
    for i=1:500
        Original_data(2,i*20)=Original_data(2,i*20)+random('Normal',0.2,1,1,1);
    end        

    
    for i=1:1000
        Original_data(2,i*10-3)=Original_data(2,i*10-3)+random('Normal',0.2,1,1,1);
    end        
...
C_uncondition2_3.Fy2x=Fy2x;
C_uncondition2_3.Fx2y=Fx2y;
...
C_uncondition1_3.Fy2x=Fy2x;
C_uncondition1_3.Fx2y=Fx2y;
...
%                 H2(k).C=H2(k).C*pinv(P2*P1);
        ...
        Causality1_3.I2to1= Fy2x;


        
figure;
plot(F,C_uncondition1_3.Fx2y,'r');hold on;plot(F,C_uncondition2_3.Fx2y,'b');
figure;
subplot(3,1,1);plot(Original_data(1,:));subplot(3,1,2);plot(Original_data(2,:));subplot(3,1,3);plot(Original_data(3,:))
figure;
subplot(2,1,1);plot(F,C_uncondition2_3.Fx2y,'g');hold on;plot(F,Causality2_3.I2to1,'r')
subplot(2,1,2);plot(F,C_uncondition1_3.Fy2x,'b');hold on;plot(F,Causality1_3.I2to1,'r')
...
% 
% d=[];
% for i=1:length(thetamax_ts_original)
%     d=[d,thetamax_ts-thetamax_ts_original(i)];
%     
% end
% 
% d(find(d>0.2|d<-0.2))=[];
% figure;[m,n]=hist(d,200);bar(n,m,'r');set(gca,'xlim',[-0.2 0.2]);
