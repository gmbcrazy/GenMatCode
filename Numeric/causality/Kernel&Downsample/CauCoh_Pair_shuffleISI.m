function CauCohShuffle=CauCoh_Pair_shuffleISI(path_filename,Data_name,timerange,CausalityParameter,ShuffleParameter)
...
[kernel,norm,m_idx] = makeKernel65('EXP',downsample/fs,0.001);
...
[Data_original(2,:),time]=rate_histogram(path_filename,Data_name(2).Name,timerange,1/fs,1/fs);
...
for shuffle_temp=1:shuffle_num  
    ...
    [kernel,norm,m_idx] = makeKernel65('EXP',downsample/fs,0.001);
       ...
       rate_f=conv(kernel,y_temp);
       index_s=m_idx;
       index=index_s:(index_s+length(y_temp)-1);
       rate_f=rate_f(index);
       y_temp=rate_f;
       clear rate_f;
    ...
    [coh,f_coh]=cohere(Data(1,:),Data(2,:),NFFT*2,fs_down,NFFT,0,'linear');
