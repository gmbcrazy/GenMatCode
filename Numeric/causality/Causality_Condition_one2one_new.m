function Causality=Causality_Condition_one2one_new(path_filename,Data_name,timerange,MORDER_band,NFFT,fre_band,fs)
...
%           [A,Z2]=armorf(Data([j,i,jj],:),Nr,Nl,MORDER);
            ...
            tmpi=([1:MORDER]-1)*(channel_num-1);
            [A_m2,Z2_m2]=armorf(Data([j,setdiff([1 2 3],[i,j])],:),Nr,Nl,MORDER);
            for p=1:(channel_num-1)
                for q=1:(channel_num-1)
                    MARP_m2(p,q).C=A_m2(p,tmpi+q);
                end
            end
            [H2_m2 FCoeff_m2 F_m2]=MARSpec(MARP_m2,Z2_m2,MORDER,fs,NFFT,STFreq,EDFreq); 

            P=[1 0;-Z2_m2(1,2)/Z2_m2(1,1) 1];
            Z2_m2=P*Z2_m2*P';
