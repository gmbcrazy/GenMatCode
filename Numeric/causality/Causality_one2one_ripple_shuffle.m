function Causality_shuffle=Causality_one2one_ripple_shuffle(path_filename,Data_name,ref_name,timerange,MORDER_band,NFFT,fre_band,fs,range,shuffle_num,P_value)
...
[raster1,time1,ts_origin1]=perievent_raster_file(path_filename,Data_name(1).Name,ref_name,timerange,1/fs,range);
[raster2,time2,ts_origin2]=perievent_raster_file(path_filename,Data_name(2).Name,ref_name,timerange,1/fs,range);
invalid=[];
...
raster1(invalid)=[];raster2(invalid)=[];
raster1=zscore(raster1')';raster2=zscore(raster2')';
    Nl=length(raster1(1,:));
    Nr=length(raster1(:,1));
    STFreq=fre_band(1);
    EDFreq=fre_band(2);
 O_Data(1,:)=reshape(raster1',1,Nr*Nl);
 O_Data(2,:)=reshape(raster2',1,Nr*Nl);
...
for shuffle_temp=1:shuffle_num  
    temp_raster1=raster1;
    temp_raster2=raster2;
...
%   temp_raster1(:,resample1)=temp_raster1(:,resample_index);  
%   temp_raster2(:,resample2)=temp_raster2(:,resample_index);  
 Data(1,resample1)=O_Data(1,resample_index);
 Data(2,resample2)=O_Data(2,resample_index);
    clear temp_raster1 temp_raster2
% MORDER_max=60;
