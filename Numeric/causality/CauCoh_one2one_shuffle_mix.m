function CauCohShuffle=CauCoh_one2one_shuffle_mix(path_filename,Data_name,timerange,MORDER_band,NFFT,fre_band,fs,shuffle_num,P_value,trial_length)
...
%     fs=1000;
    ...
    Data_o=[Data1_o Data2_o];
for shuffle_temp=1:shuffle_num  
    tic
    resample_index=1:2*trial_num;
    resample1=shuffle(resample_index);

    Data(1,:)=reshape(Data_o(:,resample1(1:trial_num)),1,trial_num*trial_length);
    Data(2,:)=reshape(Data_o(:,resample1((trial_num+1):2*trial_num)),1,trial_num*trial_length);
