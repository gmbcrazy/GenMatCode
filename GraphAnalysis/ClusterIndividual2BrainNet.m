function ClusterIndividual2BrainNet(Path,File,EdgeIndex,Cluster,write2BrainNet,varargin)

%%%%
if nargin==6
   RawSigMatrix=varargin{1}; 
end
...
fclose(fid);
    EdgeMatrixO=EdgeMatrix;

for j=1:max(Cluster)

    EdgeMatrix=zeros(size(EdgeMatrixO));
    EdgeMatrix(EdgeMatrixO==j)=1;
    E1=RawSigMatrix;
    E1(EdgeMatrix~=1)=0;
    EdgeMatrix=E1;
fid = fopen([Path '\' File num2str(j) '.EDGE'],'w');
for i = 1:size(EdgeMatrix,1)
