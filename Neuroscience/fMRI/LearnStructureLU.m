function [MBnet, FullBN, outstats] = LearnStructureLU(data, cols, pheno, priorPrecision, outfilename, ...
    verbose, disc)
...
if (isfield(priorPrecision,'BFTHRESH'))
   ...
   searchParameter.Back_Step_Mult=true;
   searchParameter.nophenotype=false;


[BN,outstats]=ExhaustiveFullNetSearch(contData,discData,priorPrecision,max(find(disc==1)) ,searchParameter); 
%     figure;
%     subplot(1,2,1)
%     [x, y, h] = draw_graph(BN.adjMatrix, cols);
...
% % % % % % % % %    cols=cols([tempIdx ])
    ...
    [BN,outstats]=ExhaustiveFullNetSearch(contData,discData,priorPrecision,max(find(disc==1)) ,searchParameter); 
%     subplot(1,2,2)
%     [x, y, h] = draw_graph(BN.adjMatrix, cols([tempIdx 7]));
