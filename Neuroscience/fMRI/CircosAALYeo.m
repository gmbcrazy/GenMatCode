function CircosAALYeo(Data,PathData,Pth,MCcorrection,PlotAll,varargin)
...
for i=1:size(LobleID,1)
    temptext=['al' num2str(LobTemp(i)) ' ' num2str(IDTemp(i)-step+1) ' ' num2str(IDTemp(i)-1) ' ' num2str(Yeo2011(i,j)) ' fill_color=Yeo'];
    Show(i,1:length(temptext))=temptext;
end
for i=1:length(Show(:,1))
    fprintf(fileID,'%s\r\n',deblank(Show(i,:)));
