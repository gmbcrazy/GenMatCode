function [spkx,spky,spkInd,VariInd] = EgoSpikePosLu(ts,posx,posy,post)
...
for ii = 1:N
    ...
    if (ind-1)>0&&(ind+1)<=length(posx)
    else
        continue
    end
    % Check if spike is in legal time sone
    if (~isnan(posx(ind-1)))&&(~isnan(posx(ind+1)))&&(~isnan(posx(ind)))
        count = count + 1;
        spkx(count) = posx(ind+1)+posx(ind-1)-posx(ind)*2;
        spky(count) = posy(ind+1)+posy(ind-1)-posy(ind)*2;
        spkInd(count) = ind(1);
        VariInd(count)=ii;
    end
    currentPos = ind;
