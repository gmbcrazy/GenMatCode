function Output=GT_PhasePrecessionPlotEXGroup(Pos,Phase,SpikeID,IDCluster,BinNum,PosInt,PlotMode,ColorP,varargin)
%%%%%%%Pos: Spike Position
%%%%%%%Pos: Spike Phase
...
switch PlotMode
    ...
    case 1
        for i=1:length(IDCluster)
            ...
            x3=repmat(SpikeID(Index),2,1);
            for j=1:length(IDCluster)
                IndexP=find(x3==IDCluster(j));
               if ~isempty(IndexP)
                plot(x1(IndexP),x2(IndexP),'.','color',ColorP(j,:));
                hold on;
               end
            end
           plot([0 1],a(i)*[0 1]+b(i),'color','k');
            plot([0 1],a(i)*[0 1]+b(i)+2*pi,'color','k');
           plot([0 1],a(i)*[0 1]+b(i)+4*pi,'color','k');
