...
% was: expY = nansum(Y.^2);
% now: expY = nansum(nansum(nansum(MEANS.^2)));
%
...
A = nansum(AB,2); % nansum across columns, so result is ax1 column vector
B = nansum(AB,1); % nansum across rows, so result is 1xb row vector
S = nansum(AS,1); % nansum across columns, so result is 1xs row vector
T = nansum(nansum(A)); % could nansum either A or B or S, choice is arbitrary
...
expY = nansum(nansum(nansum(MEANS.^2))); %nansum(Y.^2);
expT = T^2 / (a*b*n);
