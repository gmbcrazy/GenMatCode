...
%             set(sh{1}(ishandle(sh{1})),'color',[0,128,255]/255);
            set(sh{1}(ishandle(sh{1})),'color',[0.6 0.6 0.6]);
...
if opt.showMM
    ...
    switch opt.xyOri
        case 'normal'
            ...
            if opt.showMM == 4
%                 mh = plot(ah,opt.xValues+xOffset,m,'+r','Color','r','MarkerSize',12);
                xStep=mean(diff(opt.xValues))/8;
                for imm=1:length(m)
                    plot(opt.xValues(imm)+[-xStep xStep],repmat(m(imm),1,2),'-','Color',opt.color{1},'linewidth',2); 
                    
                end
                
                mdh = myErrorbar(ah,opt.xValues+xOffset+xStep*0,m,sem);
