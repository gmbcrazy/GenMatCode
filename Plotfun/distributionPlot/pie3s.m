    ...
    for ii=1:numel( params )
        switch upper( params{ii} )
            ...
            case 'BITE'
                bite = (values{ii} ~= 0);
            case 'COLOR'
                ColorGroup = values{ii};
                ColorGroup(InvalidIndex,:)=[];
            otherwise
...
%             colour, ...
    ...
    if ~isempty(ColorGroup)
        set(pieSegment,'facecolor',ColorGroup(ii,:));
%         set(pieSegment,'linecolor',ColorGroup(ii,:));
        set(pieSegment,'Edgecolor',ColorGroup(ii,:));
        set(pieSegment,'markeredgecolor',ColorGroup(ii,:));
        set(pieSegment,'markerfacecolor',ColorGroup(ii,:),'EdgeLighting','flat','linestyle','none');
