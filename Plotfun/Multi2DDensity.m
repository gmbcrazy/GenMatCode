function a=Multi2DDensity(xx,yy,XBin,YBin,ComColor,varargin)   
...
%  XYBin=XYBin(:);
if isnumeric(xx)
    H = hist2dLU([xx(:),yy(:)],XBin(:),YBin(:));
    HistData=SmoothDec(H,[2,2]);
...
%     AlphaTemp(AlphaTemp<=Clim(1))=0;
    ...
    a=surf(XBin,YBin,AlphaTemp,'facecolor',ComColor(1,:));grid off
    ...
    for i=1:length(Phase)
        a(i)=Multi2DDensity(xx{i},yy{i},XBin,YBin,ComColor(i,:),Param);
    end
end
