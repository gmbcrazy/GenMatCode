function [OutPut,r,p]=LuPairPartialRegressPlot(data1,data2,Cov,varargin)    
...
[B,BINT,R,RINT,STATS]=regress(data2,[ones(length(data1),1) data1 Cov]);
x=[min(data1(:)) max(data1(:))]';
y=[[1 1]' x]*B(1:2);

% S=regstats(data2,[data1 Cov],'linear');
...
[r,p]=partialcorr(data1,data2,Cov,'rows','complete','type',Param.Rtype);
...
% hold on;plot(x,y,'-','color',Param.Color,'linewidth',1); 
